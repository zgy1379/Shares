...
namespace astar_planner
{
    ...
    vector<int> AstarPlanner::get_neighbors(int current_cell)
    {
        ...
        if (current_cell + width - 1 < width * height && (current_cell + width - 1 + 1) % width != 0)
        {
        ...
        if (current_cell + width + 1 < width * height && (current_cell + width + 1) % width != 0)
        {
            neighborIndexes.push_back(current_cell + width + 1); // right_down
        }
        if (current_cell + 1 < width * height && (current_cell + 1) % width != 0)
        {
            neighborIndexes.push_back(current_cell + 1); // right
        }
        if (current_cell - width + 1 >= 0 && (current_cell - width + 1) % width != 0)
        {
