...
class ROSNavNode(object):
    ...
    def get_state(self):
        ...
        reach_flag = False
        if (self.info.pose.pose.position.x > -2.62) and (self.info.pose.pose.position.x < -0.9) and (self.info.pose.pose.position.y > 1.0) and (self.info.pose.pose.position.y < 1.35):
            reach_flag = True
        if reach_flag:
            return True
        else:
